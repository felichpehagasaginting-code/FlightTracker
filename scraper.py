...
class FlightScraper:
    ...
    def search_date(self, date_str: str) -> List[Dict[str, Any]]:
        """
        Melakukan ekstraksi data REAL-TIME dari Google Flights untuk rute KNO -> CGK (Sekali Jalan).
        TIDAK ADA MOCK DATA SAMA SEKALI.
        """
        print(f"🌐 [REAL DATA] Mengambil data penerbangan live dari Google Flights: {ORIGIN} -> {DESTINATION} ({date_str})...")
        return self._scrape_google_flights_real(date_str)
